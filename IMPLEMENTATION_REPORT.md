...
## 15. Addendum — Phase 22: Payments & Financial Ecosystem Integration (Version 2.43.0)

_Added 2026-07-24, same day as §13/§14. Covers HEAD commit at time of writing (post-Version 2.42.0)._

### 15.1 Theme

Closed the last gap `MASTER_DEVELOPMENT_ROADMAP.md`'s Part 1 named ("the business runs on it end-to-end, including money and hardware"): Customer Portal (Phase 14) was read-only, Finance data never left the system without manual re-entry, and Measurement/Installation sign-off relied on manually uploaded signature images. All three items Phase 22 named are closed, each behind the same provider-abstraction discipline Phase 21 established for AI and Phase 7 established for Communication — mock-by-default, one real integration behind the interface, no use case/DTO/schema change to swap providers.

### 15.2 Features Added

- **`PaymentGatewayProvider`** (`modules/finance/infrastructure/providers/`) — `MockPaymentGatewayProvider` (default) and `StripeProvider` (real, official `stripe` SDK's Checkout Sessions). A new `invoice_payment_sessions` table (§10.5) tracks one checkout attempt against one Invoice's outstanding balance, separate from `invoice_payments` so an abandoned/failed checkout never touches `amount_paid`/`status`. `CreatePaymentSessionUseCase` (Customer Portal's first-ever write action, `POST /customer_portal/me/invoices/{id}/pay`) validates the invoice is payable and attributable, then calls the provider; `HandlePaymentGatewayWebhookUseCase` (`POST /finance/payments/webhooks/{provider}`, public, signature-verified) is the only path that ever resolves a session to a real `Payment`, via the completely unchanged `RecordPaymentUseCase` — never trusted from the browser's own success-redirect, since a closed tab or a forged URL could otherwise fake a completed payment. `SimulatePaymentSessionUseCase` (mock-only) makes the whole flow demoable with no real Stripe account; a new `/portal/pay/[sessionId]` page is the mock's own simulated checkout page.
- **Accounting/ERP export** (`modules/finance/presentation/api/export.py`) — `GET /finance/export/{invoices|payments|expenses|journal}`, CSV with a UTF-8 BOM, matching Purchasing's existing `GET /export/{resource}` convention exactly (same helper shape, new `finance:export:read` permission). The `journal` resource is the genuinely "accounting" deliverable: a synthesized double-entry-style export mapping Invoices→Accounts Receivable/Sales Revenue, Payments→Cash-or-Bank/Accounts Receivable, and Expenses→Expense:{category}/Cash into `date, account, debit, credit, reference, memo` rows any accounting system (1C and similar are the norm for these three companies) can import as journal entries — not just a raw table dump. A new "Accounting Export" button group on `/finance/invoices`.
- **`core/esignature/`** — a new *stateless provider library living in core*, not inside either consuming module: `ESignatureProvider` interface, `MockESignatureProvider` (default), and `DropboxSignProvider` (real, official `dropbox_sign` SDK, hosted/emailed signing rather than embedded — no OAuth API-App `client_id` or frontend JS SDK needed, keeping the integration entirely backend-driven). This is a deliberate departure from the `AIProvider`/`ChannelProvider` precedent (a provider abstraction owned by the one module that uses it): e-signature is the first capability two independent modules (Sales, Installation) both need, and core is the correct home for genuinely cross-cutting, business-agnostic infrastructure — exactly the same reasoning that already put `core/storage`'s Document client there. Core still never imports either module; each module calls the library directly and tracks its own request state on its own new columns.
- **Sales measurement sign-off** — three new columns on `sales_project_item_measurements` (§6.6): `signature_status`/`signature_provider`/`signature_provider_request_id`. `RequestMeasurementSignatureUseCase` builds a one-page PDF summary (reportlab) and sends it via `core.esignature`; completion (`SimulateMeasurementSignatureUseCase` for mock, or the public webhook `POST /sales/webhooks/esignature/{provider}` for real) stores the signed PDF as a `Document` and sets the exact same `customer_signature_document_id` + `status: final` the pre-existing manual-upload path (Phase 9) already used — both paths coexist in the frontend UI.
- **Installation job completion sign-off** — the identical pattern on `installation_jobs` (§9.2): `RequestJobSignatureUseCase`/`SimulateJobSignatureUseCase`/`POST /installation/webhooks/esignature/{provider}`. Completion creates the same `photo_type="signature"` `InstallationPhoto` row the frontend's canvas `SignaturePad` capture flow already produces — the "Request E-Signature" link sits alongside the SignaturePad, not in place of it.
- **Webhook-originated audit attribution** — every payment or signature write triggered by an external callback (no authenticated staff user) is attributed to the underlying record's own `created_by` (`Invoice.created_by`, `ProjectItemMeasurement.created_by`, `InstallationJob.created_by`), raising a clear domain exception if that's unexpectedly `None` — the identical guard Communication's channel-webhook handling already established (`webhook_use_cases.py`'s `_load_channel_and_credential`), applied here for the first time outside Communication.
- Both new SDKs (`stripe==15.3.1`, `dropbox-sign==1.11.0`) added to `requirements.txt`, matching the `anthropic` SDK precedent from Phase 21 rather than hand-rolled HTTP against either vendor's REST API.

### 15.3 Tests Added

| Area | New tests |
|---|---|
| `tests/core/test_esignature_provider.py` | 5 — registry defaults to mock / resolves an explicit name; mock provider send+verify round-trip; `DropboxSignProvider` raises when unconfigured; rejects an invalid webhook signature |
| `tests/finance/test_payment_sessions.py` | 4 — `StripeProvider` raises when unconfigured / rejects an invalid webhook signature; `CreatePaymentSessionUseCase` rejects a fully-paid invoice / an invoice with no `created_by` to attribute to |
| `tests/finance/test_export.py` | 6 — invoices/payments/expenses CSV shape and content; journal export's debits equal its credits; unknown resource 404s; requires auth |
| `tests/customer_portal/test_payments.py` | 7 — session creation defaults to mock; a second customer can't pay someone else's invoice; a draft invoice can't be paid; simulate completes/fails correctly; session status poll; webhook completes the invoice and is idempotent on a retried delivery |
| `tests/sales/test_measurement_signature.py` | 8 — mock provider by default; requires a customer email; simulate completed sets the document + `final` status; simulate declined sets no document; simulating without a pending request 400s; webhook completes and is idempotent; unknown webhook request id is a no-op; requires write permission |
| `tests/installation/test_job_signature.py` | 8 — the identical 8 cases as Sales' measurement signature tests, applied to `InstallationJob`/`InstallationPhoto` |

No frontend unit-test framework exists in this codebase (consistent with §5/§9.3/§10.3/§11.3/§12.3/§13.3/§14.3); frontend changes verified via `tsc --noEmit`, `npm run lint`, and a full production build.

### 15.4 Verification

Full backend suite passing (831/831 — 793 prior + 38 new), `lint-imports` clean (`core/esignature/` imports nothing from either `modules/sales` or `modules/installation`), migration round-trip clean (`upgrade head` → `downgrade -1` → `upgrade head`, SQLite — the new `invoice_payment_sessions` table carries its own Postgres RLS policy directly, per the Phase 18/19/20/21 convention; the six new columns on `sales_project_item_measurements`/`installation_jobs` need no RLS change since RLS is row-level, already covering any new column on an already-covered table), frontend `tsc --noEmit` clean, `npm run lint` clean, frontend production build clean (67 routes, +1 new: `/portal/pay/[sessionId]`).

### 15.5 Remaining Issues (re-verified, current as of this addendum)
...
| — | Batch optimization runs synchronously in the request/response cycle | Still open — tracked since Phase 20 (§12.5), part of Phase 24's background job queue |
| — | Rate limiting is in-process only | Still open — tracked since Phase 21 (§13.5), part of Phase 24 |
| — | Payment gateway/e-signature webhooks assume a single, trusted provider identity per URL path segment — no per-webhook IP allowlisting or rate limiting beyond what the generic app-level middleware already provides | New, minor — same acceptable-risk level this codebase already accepts for Communication's channel webhooks |
| — | Mobile client | Not started — Phase 25 |

Full detail is in `CHANGELOG.md` [2.43.0].
