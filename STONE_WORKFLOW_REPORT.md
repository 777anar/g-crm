...
**Update 2026-07-23 (Version 2.38.0): the first eight items below were the deliberately-scoped Phase 1/2 boundary this section named, and are now closed by Phase 19 (Stone Fabrication Workflow, Phase 3) — see `MASTER_DEVELOPMENT_ROADMAP.md` Phase 19, `CHANGELOG.md` [2.38.0], and `IMPLEMENTATION_REPORT.md` §11 for the full record. Left below, annotated rather than deleted, per this project's practice of recording corrections instead of silently rewriting history. Only "Mobile client parity" remains genuinely open (tracked as `MASTER_DEVELOPMENT_ROADMAP.md` Phase 25).**

Honest gaps left open in this phase, by scope decision rather than oversight:

- ~~**No Kanban/board view for Production Jobs by stage.**~~ ✅ **Closed in Phase 2** (Version 2.35.0, the Production Planning Dashboard, `/reports/production-planning` — this line was already stale by the time Phase 2 shipped in the same report window).
- ~~**No drag-and-drop stage reordering.**~~ ✅ **Closed in Phase 19** — `/production/stages` gained move-up/move-down buttons.
- ~~**No reservation UI outside the Production Job page.**~~ ✅ **Closed in Phase 19** — `/catalog/slabs` (bulk reserve), `/catalog/reservations` (company-wide browse + release), and `/orders/{id}` (per-order Reserved Slabs card).
- ~~**`sold` and `consumed` now coexist as two valid terminal states** with overlapping real-world meaning...~~ ✅ **Closed in Phase 19** — `consumed` is now reachable only via Production's own completion cascade (`system_triggered`); a manual PATCH to `consumed` is rejected. Selling/scrapping a still-reserved slab now auto-releases its dangling reservation.
- ~~**No offcut-area or dimension validation.**~~ ✅ **Closed in Phase 19** — offcut dimensions are checked against the parent slab's own recorded dimensions (both orientations) before creation.
- ~~**No notifications or alerts on priority/stage change.**~~ ✅ **Closed in Phase 19** — a new Production notification subsystem fires on urgent-priority, stage-change, and operator-assignment moments for the assigned operator, surfaced on the Dashboard.
- ~~**Permissions stayed coarse.**~~ ✅ **Closed in Phase 19** — `production:priority:write`/`production:operator:write`/`production:stage:write` added alongside `production:write`.
- ~~**No bulk operations.**~~ ✅ **Closed in Phase 19** — bulk slab reservation and bulk stage movement, both as frontend `Promise.allSettled` fan-outs matching the Customers bulk-action precedent (confirmed to be this codebase's only bulk-action pattern; no backend "bulk endpoint" convention exists to depart from).
- **Mobile client parity is unvalidated**, same pre-existing caveat as the rest of this application — nothing in this phase was designed against or tested with an actual mobile client. Still open; tracked as `MASTER_DEVELOPMENT_ROADMAP.md` Phase 25.

None of these were regressions or defects in what shipped — they were the deliberately-scoped boundary of "Phase 1," picked up by Phase 19 with full context of what was already in place.
