...
# refuses to re-mount under a fresh empty-prefix wrapper). Both stages_router
# (`/stages`) and production_job_router (`/{work_order_id}/job` etc.) must
# be matched *before* that catch-all -- a request to `/stages` structurally
# also matches the single-segment `/{work_order_id}` pattern, and Starlette
# resolves ambiguous routes in registration order. Splicing the route lists
# directly (rather than calling .include_router(), which appends to the
# end) puts the more specific routes first.
