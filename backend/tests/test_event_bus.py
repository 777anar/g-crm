...
def test_publish_calls_subscribed_handlers(monkeypatch):
    bus = EventBus()
    monkeypatch.setattr(bus, "_persist", lambda event, db: None)
...
def test_publish_rejects_event_without_company_id(monkeypatch):
    ...
    with pytest.raises(ValueError):
        bus.publish(Event(name="LeadCreated", company_id=None, payload={}, published_by_module="crm"), db=None)
...
def test_handler_exception_does_not_break_other_handlers(monkeypatch):
    ...
    bus.publish(Event(name="OrderApproved", company_id=uuid.uuid4(), payload={}, published_by_module="sales"), db=None)
