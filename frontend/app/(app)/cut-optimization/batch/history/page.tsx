...
import { listBatchCutOptimizationRuns } from "@/lib/api/cut-optimization";
...
export default function BatchCutOptimizationHistoryPage() {
  const t = useTranslations("cutOptimization");
  const tCommon = useTranslations("common");
  const tNav = useTranslations("nav");
  const router = useRouter();

  const [runs, setRuns] = useState<CutOptimizationBatchRun[] | null>(null);
  const [nextCursor, setNextCursor] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback((options: { append?: boolean; cursor?: string } = {}) => {
    listBatchCutOptimizationRuns({ cursor: options.cursor })
      .then((r) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        <h1 className="text-xl font-semibold text-text-primary">{t("batchHistoryTitle")}</h1>
        <p className="text-sm text-text-secondary">{t("batchHistorySubtitle")}</p>
      </div>
          ...
          <div className={tableScrollShellClass}>
            <table className="w-full text-left text-sm">
              <thead className={stickyTheadClass}>
                <tr>
                  <th className="px-4 py-2 font-medium">{t("slabsUsed")}</th>
              ...
              <tbody>
                {runs.map((run) => (
                  <tr
                    key={run.id}
                    onClick={() => router.push(`/cut-optimization/batch/history/${run.id}`)}
                    className="cursor-pointer border-b border-border last:border-0 hover:bg-bg"
                  >
                    <td className="px-4 py-2 font-mono text-text-primary">{run.slabs_used_count}</td>
