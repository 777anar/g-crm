...
### Added
- **Reservation UI** — `catalog:slabs:write` users can now reserve one or more `available` slabs directly from `/catalog/slabs` (checkbox selection + a bulk "Reserve selected" toolbar posting `order_id`/`order_item_id`, `Promise.allSettled` with partial-failure toasting, mirroring the Customers bulk-action pattern). A new company-wide `/catalog/reservations` page browses every reservation (not just per-order), filterable by status, with a Release action — backed by a new `GET /catalog/reservations` shape: `order_id` is now optional (omit it to browse the whole company) and the endpoint gained cursor pagination + a `status` filter. The Order detail page (`/orders/{id}`) gained a "Reserved Slabs" card listing that order's reservations with its own Release action.
- **Drag-and-drop stage movement** — the Production Planning Dashboard's Kanban board (`/reports/production-planning`) is now a real interaction surface: job cards are draggable onto a different stage column (native HTML5 drag-and-drop, no library), and a checkbox-based multi-select + "Move selected to stage" toolbar handles bulk moves — both call the existing `POST /production/{id}/stage` endpoint per job (`Promise.allSettled`), gated by the new `production:stage:write` permission.
- **Stage reordering UI** — `/production/stages` gained move-up/move-down buttons per stage, swapping `sort_order` with the adjacent stage via two `PATCH` calls (no new backend endpoint needed — `sort_order` was already a settable field, just never exposed in the UI).
- **Priority/stage-change notifications** — a new Production notification subsystem (`production_notifications` table, mirroring Installation's `notify_crew` pattern exactly via a new `notify_user` helper) fires when: a work order is marked `urgent` and has an assigned operator; a work order's stage changes and it has an assigned operator (deliberately not keyed to a specific stage name like "Quality Control," since stage names are per-company configurable and a hardcoded match would silently break on rename); an operator is newly assigned to a work order. `GET/POST /production/notifications` mirrors Installation's list/mark-read shape exactly. Surfaced on the Dashboard as a third source merged into the existing task/installation notification feed.
- **Bulk operations** — both new bulk flows above (reserve multiple slabs, move multiple jobs to a stage) are pure frontend `Promise.allSettled` fan-outs over existing single-item endpoints, matching this codebase's established convention (confirmed via the Customers bulk-action precedent — there is no backend "bulk endpoint" pattern anywhere in this codebase to depart from).
- **Finer-grained production permissions** — `production:priority:write`, `production:operator:write`, and `production:stage:write` replace the single coarse `production:write` for those three specific tracking endpoints (create/status/stage-config endpoints keep `production:write`). Purely additive: every existing role still passes via the same rep-tier default (the action suffix is still `:write`), but a company can now grant/restrict one of the three independently via a user's `module_permissions` override — e.g. letting a rep reassign operators without letting them re-prioritize.
- **Offcut dimension/area validation** — `POST /catalog/slabs/{id}/offcuts` now rejects (`422`) an offcut whose `length_mm`/`width_mm` couldn't plausibly fit within its parent slab's own recorded dimensions, checked in both orientations (a cut piece is routinely rotated relative to the parent). Silently skipped when either slab is missing a dimension, since it's a plausibility check, not a physical measurement.
- **`sold` vs. `consumed` reconciliation** — `consumed` can now only be reached by Production's own work-order-completion cascade (a new `system_triggered` flag on `UpdateSlabStatusUseCase`, defaulted `False` everywhere except that one internal call site); a manual `PATCH .../status {"status":"consumed"}` is rejected (`409`). Separately, manually transitioning a `reserved`/`in_production` slab to `sold` or `scrap` now auto-releases its dangling active reservation instead of leaving the `SlabReservation` row stuck `active` forever — the concrete data-quality gap the roadmap flagged, closed without changing the existing `reserved → sold` transition's legality (the accepted existing test for it still passes unmodified).
- 15 new backend tests (`tests/catalog/test_phase19_offcut_validation_and_status_boundary.py`, `tests/production/test_phase19_permissions_and_notifications.py`).

### Verification
Full backend suite passing (737/737 — 722 prior + 15 new), migrations round-trip clean (`upgrade head` → `downgrade -1` → `upgrade head`, SQLite), frontend `tsc --noEmit` clean, `npm run lint` clean, frontend production build clean (60 routes, +1 new: `/catalog/reservations`).
...
### Added
- **Customer archive restore (B6)** — `RestoreCustomerUseCase` (`modules/crm/application/use_cases/customer_use_cases.py`), `POST /api/v1/crm/customers/{id}/restore` (`crm:customers:write`, `409` if not archived), a new `CustomerRestored` domain event and `CustomerNotArchivedError` exception, audit-logged exactly like every other write. Frontend: a "Restore" action on the Customer detail page (replacing the Archive button once archived) and a per-row quick-restore action in the Customers list's archived-status column. 5 new backend tests.
- **Finance invoice / Production work-order `sort` parameters (B1)** — both `InvoiceRepository.list()` and `WorkOrderRepository.list()` gained a whitelisted `_SORTABLE` column map and `sort` query param, exactly matching the pattern `OrderRepository.list()` already used (`invoice_number`/`status`/`created_at`/`due_date`/`total_amount` for Invoices; `work_order_number`/`status`/`created_at`/`priority`/`scheduled_start_date`/`scheduled_completion_date` for Production). Frontend: both list pages gained `SortableHeader` columns wired to the new parameter, including saved-filter persistence on Finance Invoices. 4 new backend tests.
- **Committed ESLint configuration** — `eslint@9`, `eslint-config-next@15.5.19`, and `@eslint/eslintrc` added; `eslint.config.mjs` (flat config, `next/core-web-vitals` + `next/typescript` via `FlatCompat`) makes `npm run lint` (already documented in `CLAUDE.md`, previously non-functional) actually run. Wired into `.github/workflows/ci.yml` as a new `Lint` step between typecheck and build, so a lint violation now fails CI, not just a local run someone remembers to do.
- **`lib/permissions.ts`** — `hasPermission()`/`usePermission()`, decoding the current access token's `role`/`module_permissions` claims (the same claims `core/rbac/dependencies.py` enforces against) and replicating `core/rbac/permissions.py`'s role-rank + action-suffix convention client-side. A UX affordance only — every write endpoint still enforces `require_permission` independently regardless of what this returns. Applied as the demonstrated pattern to the Customers list (Create button, bulk-action bar and its selection checkboxes, per-row Restore action) and the Customer detail page (Archive/Restore button), closing the specific "viewer sees the same Create/Edit/Archive buttons as an owner" gap named in `PROJECT_AUDIT.md`.
- **`tests/test_database_schema.py`** — a new top-level backend test guarding against the circular-FK regression class below: fails loudly if `Base.metadata.sorted_tables` ever raises an "unresolvable cycles" `SAWarning` again, and pins down the specific `use_alter`/constraint-name fix so a future refactor can't silently drop it.

### Fixed
- **Circular FK warning (B5)** — `crm_customers.primary_contact_id`'s `ForeignKey("crm_contacts.id")` gained `use_alter=True, name="fk_crm_customers_primary_contact_id"`, the standard SQLAlchemy resolution for a mutual (`crm_customers` ↔ `crm_contacts`) foreign-key reference. Confirmed via `alembic check` that **no migration is required** — the already-migrated database's constraint is unaffected; this was purely a metadata/table-sort-ordering fix, not a schema change. Verified the "unresolvable cycles" `SAWarning` no longer fires when sorting the full 14-module table set.
- **Dead frontend data (B7)** — `module_permissions`, previously fetched by `me()` on every session and never read anywhere, is now genuinely consumed by `lib/permissions.ts` (see above) rather than removed, since the data is exactly what a permission-gating hook needs.
- **Tablet breakpoint** — `AppShell` gained a third responsive tier: a persistent icon-only collapsed sidebar (`NavIconRail`, new) at `md` to `<lg` (768–1023px, per `UI_UX_GUIDELINES.md` §7), distinct from the `<md` phone-width slide-over drawer (previously tablet widths fell all the way back to that drawer) and the `>=lg` full labeled sidebar.
- **Mobile nav focus trap** — a new `useFocusTrap` hook (`lib/use-outside-click.ts`) traps Tab/Shift+Tab inside the mobile slide-over drawer while it's open (previously keyboard focus could escape into the page content behind it) and returns focus to the trigger button on close.
- **Icon library standardization** — `lucide-react` added as the single icon dependency `UI_UX_GUIDELINES.md` §2 always called for; every hand-rolled inline `<svg>` icon (app-shell hamburger/close/6 nav icons, theme toggle sun/moon, company/language switcher chevrons, quick-create plus, data-table column-visibility icon — 9 icons across 6 files) replaced with the equivalent Lucide component, same size/stroke-width/`aria-hidden` conventions preserved.
- **93 pre-existing `@typescript-eslint/no-explicit-any` violations**, surfaced by turning ESLint on for the first time — 89 were the identical pattern (`t(dynamicKey as any)`, an `any` cast satisfying `next-intl`'s literal-keyed `t()` signature against a runtime-computed translation key) across 36 files, mechanically replaced with the type-scoped `as Parameters<typeof t>[0]` equivalent — same runtime behavior, verified with `tsc --noEmit` after. The remaining 4 were genuine free-form-JSON fields (`AIRecommendation.response`/`edited_response` in `lib/types.ts`, `reviewRecommendation`'s `editedResponse` param in `lib/api/ai.ts`) retyped `Record<string, unknown>`, and one fully dead state variable (`communication/inbox`'s unused `currentUserId`, set from `me()` and never read) removed outright.
- **`jsx-a11y/role-supports-aria-props`** on `SortableHeader` — `aria-sort` was set on the inner `<button>`, which doesn't support that attribute per the ARIA spec; moved to the `<th>` itself, the correct target for a sortable column header.
- A handful of genuinely dead imports/variables ESLint's `no-unused-vars` surfaced (`useCallback` and an unused `toast` in Cut Optimization's tool page, an unused `tCommon` in Production Stages, an unused `CardHeader` import in Sales Projects, an unused `REPORT_PERIODS` import in the shared date-range filter, a stale `eslint-disable-next-line react-hooks/exhaustive-deps` directive with nothing left to suppress) — all removed.

### Verification
Full backend suite passing (706/706 — 695 prior + 11 new), `lint-imports` passing (1 contract kept, 0 broken), `alembic check` clean (no migration needed for the FK fix), frontend `tsc --noEmit` clean, `npm run lint` clean (0 errors, 0 warnings — genuinely fixed, not suppressed), frontend production build clean (68 routes, unchanged route count — this was a stabilization pass, not a feature release).
